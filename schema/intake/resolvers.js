...
const intakeResolvers = {
  ...
  Mutation: {
    saveIntake: async (parent, args) => {
      const unique_id = generateUniqueID();
      const { id, intake_title, added_by } = args;
      ...
      if (id) {
        // update
        try {
          let sql = `UPDATE intakes SET intake_title = ?, modified_by = ?, modified_on = ? WHERE id = ?`;

          let values = [intake_title, added_by, today, id];
          ...
          if (results.affectedRows == 0) {
            ...
            throw new GraphQLError(`No intake with id ${id}`, {
              extensions: {
                // code: '',
                http: { status: 400 },
              },
        ...
        // create new record
        try {
          let sql = `INSERT INTO intakes(id, intake_title, added_on, added_by) VALUES (?, ?, ?, ?)`;

          let values = [unique_id, intake_title, today, added_by];
      ...
      return results[0]; // returning the inserted intake
    },
  },
};

export default intakeResolvers;
